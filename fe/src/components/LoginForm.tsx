...
export function LoginForm({
  ...
  const { login, redirectBasedOnRole } = useAuthStore();
  ...
  const onSubmit = async (value: LoginForm) => {
    try {
      ...
      const user = useAuthStore.getState().userDetails;
      if (user) {
        showToastSuccess(`Welcome back, ${user.name || "user"}!`);
        redirectBasedOnRole(user.role);
      } else {
        showToastError("Login failed: No user data returned.");
      }
