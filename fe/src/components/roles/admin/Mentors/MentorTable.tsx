"use client";

import React from "react";
import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow
} from "@/components/ui/table";
import { Button } from "@/components/ui/button";
import { Edit, Trash2 } from "lucide-react";
import Link from "next/link";
import { User } from "@/types/user.type";
import { cn } from "@/lib/utils";

type Props = {
  mentors: User[];
  pageSize: number;
  onEdit: (user: User) => void;
  onDelete: (user: User) => void;
};

export const MentorTable: React.FC<Props> = ({
  mentors,
  pageSize,
  onEdit,
  onDelete
}) => {
  return (
    <div className="overflow-x-auto">
      <Table>
        <TableHeader>
          <TableRow className="bg-gradient-to-r from-slate-50 to-slate-100/80">
            <TableHead className="w-[150px] table-header">Full Name</TableHead>
            <TableHead className="w-[150px] table-header">UserName</TableHead>
            <TableHead className="w-[150px] table-header">Email</TableHead>
            <TableHead className="w-[100px] text-center table-header">
              Phone
            </TableHead>
            <TableHead className="w-[100px] text-center table-header">
              Status
            </TableHead>
            <TableHead className="w-[100px] text-center table-header">
              Assigned
            </TableHead>
            <TableHead className="w-[100px] text-center table-header">
              Actions
            </TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {mentors.map((m) => (
            <TableRow key={m.id} className="hover:bg-blue-50/50 transition-colors duration-150">
              <TableCell className="underline">
                <Link href={`/admin/mentors/${m.id}`}>{m.fullName}</Link>
              </TableCell>
              <TableCell>{m.username}</TableCell>
              <TableCell>{m.email}</TableCell>
              <TableCell className="text-center">
                {m.phoneNumber ?? "-"}
              </TableCell>
              <TableCell className=" text-center">
                <span
                  className={cn(
                    "px-2 py-1 rounded-full text-xs font-medium",
                    m.status === "active"
                      ? "bg-green-100 text-green-800"
                      : "bg-red-100 text-red-800"
                  )}
                >
                  {m.status}
                </span>
              </TableCell>
              <TableCell className=" text-center">
                {m.isAssigned ? "Yes" : "No"}
              </TableCell>
              <TableCell className="flex gap-2">
                <div className="m-auto flex gap-1">
                  <Button variant="ghost" size="icon" onClick={() => onEdit(m)} className="hover:bg-blue-100 rounded-lg transition-colors">
                    <Edit size={16} className="text-blue-600" />
                  </Button>
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => onDelete(m)}
                    className="hover:bg-red-100 rounded-lg transition-colors"
                  >
                    <Trash2 size={16} className="text-red-600" />
                  </Button>
                </div>
              </TableCell>
            </TableRow>
          ))}
          {Array.from({
            length: pageSize - mentors.length
          }).map((_, index) => (
            <TableRow key={`empty-${index}`}>
              <TableCell colSpan={7} className="h-12 " />
            </TableRow>
          ))}
        </TableBody>
      </Table>
    </div>
  );
};
